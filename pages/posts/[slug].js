...
export default function PostPage({ source, frontMatter, prevPost, nextPost, globalData }) {
    return (
        <Layout>
                ...
                <div className='grid md:grid-cols-2 lg:-mx-24 mt-12'>
                    {prevPost && (
                        <Link href={`/posts/${prevPost.slug}`}>
                            <strong className='py-8 px-10 text-center md:text-right first:rounded-t-lg md:first:rounded-tr-none md:first:rounded-l-lg last:rounded-r-lg first last:rounded-b-lg backdrop-blur-lg bg-white dark:bg-black dark:bg-opacity-30 bg-opacity-10 hover:bg-opacity-20 dark:hover:bg-opacity-50 transition border border-gray-800 dark:border-white border-opacity-10 dark:border-opacity-10 last:border-t md:border-r-0 md:last:border-r md:last:rounded-r-none flex flex-col'>
